...
async def get_auth_token() -> str:
    ...
    has_credentials = (
        tenant_id and client_id and client_secret and
        "here" not in tenant_id and "here" not in client_id
    )
    
    if not has_credentials:
        logger.error("No valid Entra ID Client Credentials configured in .env.")
        raise HTTPException(
            status_code=500,
            detail="Authentication failed. Please configure ENTRA_TENANT_ID, ENTRA_CLIENT_ID, and ENTRA_CLIENT_SECRET in .env"
        )
    ...
    try:
        ...
        logger.error(f"[MSAL ERROR] Failed to fetch token via Client Credentials: {e}", exc_info=True)
